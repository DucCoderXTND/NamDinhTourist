...
{
    public class GetCitiesForManagementHandler : IRequestHandler<GetCitiesForManagementQuery, PaginatedList<CityForManagementResponse>>
    {
        private readonly ICityRepository _cityRepository;
        private readonly IMapper _mapper;

        public GetCitiesForManagementHandler(ICityRepository cityRepository, IMapper mapper)
        {
            _cityRepository = cityRepository;
            _mapper = mapper;
        }
        public async Task<PaginatedList<CityForManagementResponse>> Handle(GetCitiesForManagementQuery request, 
        ...
        private static Expression<Func<City, bool>> GetSearchExpression(string? searchTerm)
        {
            return searchTerm is null
                ? _ => true
                : x => x.Name.Contains(searchTerm) || x.Country.Contains(searchTerm);
        }
    }
}
