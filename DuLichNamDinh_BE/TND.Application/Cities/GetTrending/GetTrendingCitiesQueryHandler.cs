...
namespace TND.Application.Cities.GetTrending
{
    public class GetTrendingCitiesQueryHandler : 
        IRequestHandler<GetTrendingCitiesQuery, IEnumerable<TrendingCityResponse>>
    {
        ...
        public async Task<IEnumerable<TrendingCityResponse>> Handle(GetTrendingCitiesQuery request, CancellationToken cancellationToken)
        {
            if (request.Count <= 0)
